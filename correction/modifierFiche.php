<?php
	...
	//function valider(){
		...
		$q = "SELECT numero FROM origine where numero = $numero";		
					...
					if ($newFileD){
						if (preg_match("#^(image/|application/pdf$)#",mime_content_type($_FILES['fileDossier']['tmp_name']))){
							...
							if (move_uploaded_file($_FILES['fileDossier']['tmp_name'], $uploadFile)){
								...
								$good = false;
								mysqli_query($link, "update origine set archive='x' where id='$id'");
							}
						}
						else{
							echo "invalide";
	...
	// (Dans tous les cas, on obtient un message à afficher)
		...
		if (CheckSecurity("Delete")){
			$link = connexionBD();
			$id = mysqli_real_escape_string($link, $_POST['id']);
			$qdel = "SELECT actenumerises from origine where id='$id'";
			$row = mysqli_fetch_row(mysqli_query($link, $qdel));
			...
			if($good){
				if (mysqli_query($link, "UPDATE origine SET actenumerises='x' WHERE id='$id'")){
	...
	// (Dans tous les cas, on obtient un message à afficher)
		...
		if (CheckSecurity("Delete")){
			$link = connexionBD();
			$id = mysqli_real_escape_string($link, $_POST['id']);
			$qdel = "SELECT archive from origine where id='$id'";
			$row = mysqli_fetch_row(mysqli_query($link, $qdel));
			$fileD = imageFolder().$row[0];
			...
			// Si le fichier à été supprimé correctement (ou était inexistant), on met le champ de la BD à jour.
			if($good){
				if (mysqli_query($link, "UPDATE origine SET archive='x' WHERE id='$id'")){
					$message .= " (BD mise à jour)";
				}
				else{
					$message .= " (erreur dans la mise à jour de la BD: ".mysqli_error($link).")";
	...
	if ($id != 0 && !isset($_POST['bouton_delete'])){
		...
		$link = connexionBD();
		$q = "SELECT * FROM origine where id = '".mysqli_real_escape_string($link, $id)."'";
		$r = mysqli_query($link, $q);
		deconnexionBD($link);
...
<html lang="fr">
	...
	<body>
		...
		?>
		<form enctype="multipart/form-data" action="modifierFiche.php<?php if ($update) echo "?id=$id" ?>" method="post">
			<div class="submitButton"><input type="submit" name="bouton_submit" value="Sauvegarder"></div>
			<a href="liste.php" class="bouton">Retour</a><br/>
			<?php 
